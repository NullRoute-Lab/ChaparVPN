...
import com.chapar.vpn.ui.theme.ConnectingAmber
import com.chapar.vpn.ui.theme.DisconnectedRed
...
private fun HowToUseDialog(onDismiss: () -> Unit) {
    ...
    val textEn = """
        ChaparRelayVPN creates a local SOCKS5 proxy and tunnels TCP traffic through your own Apps Script + VPS relay.

        Quick setup:
        1) Prepare your VPS and keep chapar-server running.
    ...
    """.trimIndent()
    val textFa = """
        ChaparRelayVPN یک پراکسی محلی SOCKS5 می‌سازد و ترافیک TCP را از مسیر Apps Script و VPS خودتان عبور می‌دهد.

        راه‌اندازی سریع:
        ۱) VPS را آماده کنید و chapar-server را اجرا نگه دارید.
